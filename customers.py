...
def chatGPT_response_table():
    try:
        ...
        query = "SELECT RSTRNT_IND FROM ai_cstmr WHERE AI_CSTMR_START_TXT LIKE %s"
        cursor.execute(query, ('%' + customer_name + '%',))
        rstrnt_result = cursor.fetchone()
        rstrnt_ind = rstrnt_result['RSTRNT_IND'] if rstrnt_result else 'N'

        # Map wine_option to the corresponding PROMPT_TXT_X
        # Note: Frontend now sends 1-10 directly
...
# Check if any word in column_words_processed matches any item in matched_regions after preprocessing itemreg['CNTRGN_NM']
                ...
                print("first if")
                return render_template("chatGPT_response_table.html", token=1,var=html_table, rstrnt_ind=rstrnt_ind)
            else:
                print("2nd if")
        #for heading in headings:
                ...
                return render_template("chatGPT_response_table.html",paragraphs_with_links=paragraphs_with_links, rstrnt_ind=rstrnt_ind)
        else:
